...
export Knapsack, objective


struct Knapsack
    optimum # Best possible combined value of items in knapsack.
    capacity # Capacity of knapsack.
    weights # Weights of all items.
    values # Values of all items.

    # Select knapsack dataset.
    function Knapsack(option::Int64)::Knapsack
        if option == 1
            return new(295, 269,
...
# Objective function to determine the fitness of chromosome.
function objective(self::Knapsack, genes::BitVector)::Float64
    w::Float64 = dot(genes, self.weights)
    v::Float64 = dot(genes, self.values)
    return w > self.capacity ? -v : v
end
